...
function initialStateFile(): string {
  if (process.env.AGENT_STATE_PATH) return STATE_PATH;
  if (existsSync(DEFAULT_STATE_FILE)) return DEFAULT_STATE_FILE;
...
function preferIpv4LoopbackWs(url: string): string {
  try {
...
function clearStaleDeviceBinding(reason: string) {
  state = {
    ...state,
...
function agentWsUrl(): string {
  const u = new URL(serverBase + '/ws/agent');
  if (state.deviceToken) u.searchParams.set('deviceToken', state.deviceToken);
  else u.searchParams.set('pairingCode', state.pairingCode);
  return u.toString();
}
...
function emitEvent(ws: WebSocket, commandId: string, level: 'info' | 'warn' | 'error', message: string) {
  send(ws, { type: 'event', commandId, level, message, atMs: Date.now() });
}

function emitScreenshot(ws: WebSocket, commandId: string, pngBase64: string) {
...
function connect() {
  ...
  if (state.deviceToken) {
    console.log(
      '[agent] deviceToken is present: pairing-code.txt is not written. If you reset control-plane DB, the next connection may clear the token automatically.'
    );
  }
  if (!state.deviceToken) {
    writePairingCodeFile();
  ...
  if (state.deviceId) console.log(`[agent] deviceId=${state.deviceId}`);
  ...
  ws.on('message', async (raw) => {
    ...
    const msg = parsed as ServerToAgent;
