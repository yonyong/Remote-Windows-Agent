...
export type Emit = (evt: { level: 'info' | 'warn' | 'error'; message: string }) => void;
export type EmitScreenshot = (pngBase64: string) => void;

/** 浏览器：优先激活已有窗口，避免 Start-Process 再开一个新实例 */
const BROWSER_FOCUS_KEYS = new Set(['chrome', 'edge', 'firefox']);
const BROWSER_PS_PROCESS: Record<string, string> = {
  chrome: 'chrome',
  edge: 'msedge',
  firefox: 'firefox'
};
...
async function activateBrowserOrStart(key: string, launchArg: string, emit: Emit): Promise<void> {
 ...
 */
export async function capturePrimaryScreenPngBase64(): Promise<string | null> {
  const dpiCs =
    'using System;using System.Runtime.InteropServices;public static class RwaDpi{' +
...
async function executeStep(step: Step, emit: Emit, emitScreenshot: EmitScreenshot) {
  if (step.type === 'sleep') {
  ...
  if (step.type === 'sendkeys') {
    ...
    await runPowerShell(
      ['Add-Type -AssemblyName System.Windows.Forms | Out-Null', `[System.Windows.Forms.SendKeys]::SendWait('${seq}')`].join('; ')
    );
    return;
  }
  ...
  if (step.type === 'press_key') {
    ...
    await runPowerShell(
      ['Add-Type -AssemblyName System.Windows.Forms | Out-Null', `[System.Windows.Forms.SendKeys]::SendWait('${escaped}')`].join('; ')
    );
    return;
  }
  ...
  if (step.type === 'volume') {
    const vk =
      step.action === 'up' ? '0xAF' : step.action === 'down' ? '0xAE' : step.action === 'mute' ? '0xAD' : null;
    if (!vk) return;
  ...
  if (step.type === 'media') {
    const vk =
      step.action === 'next' ? '0xB0' : step.action === 'prev' ? '0xB1' : step.action === 'play_pause' ? '0xB3' : null;
    if (!vk) return;
 ...
 * 会短暂改写系统剪贴板并尽量恢复。
 */
async function typeTextImeFriendly(text: string, emit: Emit) {
  if (!text) return;
...
async function tapVirtualKey(vkHex: string) {
  const ps = [
