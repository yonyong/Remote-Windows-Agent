import { CASUAL_CHAT_SYSTEM_PROMPT, COMMAND_LLM_JSON_REPAIR_SYSTEM_PROMPT, COMMAND_LLM_SYSTEM_PROMPT } from './systemPrompt.js';
...
function usageFromOpenAiJson(data, durationMs) {
    const u = data?.usage;
...
export async function parseWithOpenAiCompatible(args) {
    const logParseFail = (content, err) => {
        const logMax = llmLogBodyMaxChars();
        args.diagnosticLog?.warn?.({
    ...
    try {
        ...
        try {
            ...
            throwWithLlmModelOutput(e2 instanceof Error ? e2.message : String(e2), fixed ? `${c1}\n---json_repair_failed---\n${fixed}` : c1, e2);
        }
    }
    if (parsed.mode === 'chat') {
        return {
