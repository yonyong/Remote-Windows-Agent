...
function mapAliasToOpenAppKey(raw) {
    const s = raw.trim().toLowerCase();
    if (!s)
        return null;
    if (/chrome|chromium|谷歌/.test(s))
        return 'chrome';
    if (/edge|edg|微软浏览/.test(s))
        return 'edge';
    if (/firefox|火狐/.test(s))
        return 'firefox';
    if (/notepad|记事本/.test(s))
        return 'notepad';
    if (/calc|计算器/.test(s))
        return 'calc';
    if (/vscode|code|代码编辑/.test(s))
        return 'vscode';
    if (/explorer|资源管理|文件管理|此电脑/.test(s))
        return 'explorer';
    if (/^wt$|windows\s*terminal|终端/.test(s))
        return 'wt';
    if (/powershell|ps\s*控制台/.test(s))
        return 'powershell';
    if (/\bcmd\b|命令提示/.test(s))
        return 'cmd';
    if (/设置|settings/.test(s))
        return 'settings';
    if (/画图|paint|mspaint/.test(s))
        return 'paint';
    if (/任务管理|task\s*mgr|taskmgr/.test(s))
        return 'taskmgr';
    const one = s.replace(/\s+/g, '');
    const known = /^(calc|notepad|explorer|cmd|powershell|wt|settings|edge|chrome|firefox|paint|taskmgr|control|mmsys|ncpa|regedit|snip|wordpad|vscode)$/;
    if (known.test(one))
        return one;
    return null;
}
/** 模型杜撰的「切换窗口」类步骤 → open_app（Windows 下再次 Start 通常可激活已运行实例） */
function coerceSwitchLikeToOpenApp(o) {
    const appRaw = o.app ?? o.name ?? o.window ?? o.browser ?? o.target;
    if (typeof appRaw !== 'string')
        return null;
...
export function extractJsonObject(text) {
    ...
    if (start === -1 || end === -1 || end <= start)
        throw new Error('llm_no_json_object');
    return JSON.parse(body.slice(start, end + 1));
}
/** 先尝试纯对话信封，否则按远程指令解析 */
export function parseLlmFlexibleOutput(args) {
    const obj = extractJsonObject(args.content);
...
export function buildCommandBundleFromLlmText(args) {
    ...
    if (steps.length === 0)
        throw new Error('llm_no_valid_steps');
