...
import { augmentLlmUserTextForForegroundHint, isExplicitRemoteOperationIntent, preprocessUserCommand, shouldSkipLlmForCheapRules } from './preprocess.js';
...
function withPreprocessHint(bundle, pre) {
    /** 闲聊/大模型回复不向用户拼接预处理说明，避免污染可见文案 */
    if (bundle.kind === 'chat')
        return bundle;
    if (!pre.trimmedFiller && !pre.truncated)
        return bundle;
...
function formatLlmFailureForUser(err) {
    const msg = err && typeof err.message === 'string' ? err.message : String(err);
    if (msg.includes('llm_no_json_object'))
        return '模型未返回可解析的 JSON（常为纯文字或 Markdown）';
    if (msg.includes('Required') && msg.includes('steps'))
        return '模型 JSON 缺少可执行步骤 steps';
    if (msg.includes('JSON Parse error') || msg.includes('Unexpected token'))
