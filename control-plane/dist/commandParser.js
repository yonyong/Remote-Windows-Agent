...
export function parseWithRules(args) {
    ...
    const refuseDanger = () => {
        steps.push({
            type: 'notify',
            message: '出于安全策略：关机、重启、注销、格式化磁盘等系统级危险操作不会通过本 Agent 自动执行。如需远程维护请在目标机器本地操作或使用企业级远程管理方案。'
        });
        bump('high');
        pushScreenshot();
    };
    ...
    // --- 3b. 口语化「看一下桌面」等 → 仅截图 ---
    if (/(看看桌面|看下桌面|看一下桌面|当前画面|桌面发我|发张图|发一下截图|给我截个图|帮我截个图)/i.test(spaced) &&
        spaced.length < 36) {
        pushScreenshot();
        return assembleCommandSpec(commandId, args.deviceId, args.nowMs, risk, steps);
    }
    ...
    if (/(切换到|切到|换到|激活).{0,14}(浏览器|谷歌浏览器|谷歌|chrome|chromium|edge|微软浏览器|火狐|firefox)/i.test(spaced) ||
        ...
        let appKey = 'edge';
        if (/chrome|chromium|谷歌/.test(spaced))
            appKey = 'chrome';
        else if (/firefox|火狐/.test(spaced))
            appKey = 'firefox';
        else if (/edge|微软浏览/.test(spaced))
            appKey = 'edge';
    ...
    if (/(音量加大|音量调高|声音大一点|调大音量|大点声|大声点|响一点|volume\s*up)/i.test(spaced)) {
        for (let i = 0; i < 5; i++)
            steps.push({ type: 'volume', action: 'up' });
        steps.push({ type: 'sleep', ms: 200 });
        pushScreenshot();
        return assembleCommandSpec(commandId, args.deviceId, args.nowMs, risk, steps);
    }
    if (/(音量减小|音量调低|声音小一点|调小音量|小声点|轻一点|volume\s*down)/i.test(spaced)) {
        for (let i = 0; i < 5; i++)
            steps.push({ type: 'volume', action: 'down' });
        steps.push({ type: 'sleep', ms: 200 });
        pushScreenshot();
        return assembleCommandSpec(commandId, args.deviceId, args.nowMs, risk, steps);
    }
    ...
    // --- 12. 在 X 里输入 Y ---
    const inAppInput = spaced.match(/在\s*(计算器|记事本|画图|cmd|命令提示符|powershell|终端|terminal)\s*(?:里|中)?\s*(?:输入|键入|打入)[:：]?\s*(.+)$/i);
    if (inAppInput) {
    ...
    steps.push({
        type: 'notify',
        message: '未能把这句话匹配成可执行的远端步骤。请补充「打开、截图、锁屏、输入」等明确操作，或发送记事本、计算器、截图等快捷词。'
    });
    return assembleCommandSpec(commandId, args.deviceId, args.nowMs, risk, steps);
}
export function assembleCommandSpec(commandId, deviceId, nowMs, risk, steps) {
    const spec = {
...
function summarizeSpec(spec) {
    const parts = spec.steps.map(summarizeStep).filter((x) => Boolean(x));
...
/** 短句、口头前缀，不要求出现「打开」 */
function matchImplicitApp(raw, spaced) {
    if (/(输入|键入|打字|type\b)/i.test(spaced))
        return null;
    if (/(打开|启动|运行|开启|开一下|开下|帮我打开|给我打开)/i.test(spaced))
        return null;
...
export function looksLikeImplicitAppOnlyCommand(raw, spaced) {
    return matchImplicitApp(raw, spaced) !== null;
}
function extractExpression(textNoSpaces) {
    const m = textNoSpaces.match(/(\d+(?:[+\-*/]\d+)+)/);
    return m?.[1] ?? null;
}
function mapZhApp(zh) {
    ...
    if (/cmd|命令提示/.test(z))
        return 'cmd';
    if (/powershell|终端|terminal/.test(z))
        return 'powershell';
    return 'notepad';
}
function extractTypedPayload(s) {
...
function extractTrailingTypeText(spaced, raw) {
    ...
    if (mB)
        return mB[1].slice(0, 500);
    const mC = raw.match(/(?:输入|键入)\s*[""]([^""]+)[""]/);
    if (mC)
        return mC[1].slice(0, 500);
    return null;
}
function matchOpenApp(spaced) {
