 ...
 */
export function parseHelpCoachGoal(raw) {
    const t = raw.trim();
    if (!t)
        return null;
    if (/^求助/.test(t)) {
    ...
    if (mEn)
        return { goal: (mEn[1] ?? '').trim() };
    return null;
}
/** 未配置大模型 Key 时的固定引导（不调用上游） */
export function helpCoachFallbackNoLlm(goal) {
    const examples = [
        '截图',
        '打开 Chrome',
        '切换到浏览器',
    ...
    const head = '【使用说明】本控制台把你说的话发给已绑定的 Windows 电脑去执行。能力主要包括：截图、打开/切换 Chrome/Edge/Firefox、打开网址、在**前台窗口**里输入文字、快捷键类组合（由解析模型拆成 sendkeys）、音量、锁屏、等待等；复杂排版类软件（如 Word）更适合在电脑上本地操作或用专用自动化方案。\n\n';
    if (!goal) {
        return (head +
