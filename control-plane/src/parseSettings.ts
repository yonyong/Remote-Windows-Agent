...
export type LlmProvider = 'zhipu' | 'openai_compatible' | 'gemini';

export type UserParseSettingsRow = {
  user_id: string;
  parse_mode: string;
  llm_provider: string;
...
const PROVIDERS: LlmProvider[] = ['zhipu', 'openai_compatible', 'gemini'];

export function parseLlmProvider(s: string | undefined | null): LlmProvider {
  const x = (s ?? 'zhipu').toLowerCase().trim();
  if (x === 'openai' || x === 'openai_compatible' || x === 'chatgpt' || x === 'deepseek') return 'openai_compatible';
  if (x === 'gemini' || x === 'google') return 'gemini';
  if (x === 'zhipu' || x === 'glm' || x === 'bigmodel') return 'zhipu';
  return 'zhipu';
}

export function parseModeFromString(s: string | undefined | null): ParseMode {
  const m = (s ?? 'rule').toLowerCase().trim();
  if (m === 'llm' || m === 'hybrid') return m;
  return 'rule';
}
...
export function normalizeLlmBaseUrl(provider: LlmProvider, raw: string | null | undefined): string {
...
function resolveApiKey(provider: LlmProvider, row: UserParseSettingsRow | undefined): string | null {
  const fromUser = row?.llm_api_key?.trim();
  if (fromUser) return fromUser;
  if (provider === 'zhipu') return process.env.ZHIPU_API_KEY?.trim() || null;
...
function resolveModel(provider: LlmProvider, row: UserParseSettingsRow | undefined): string {
  const m = row?.llm_model?.trim();
  if (m) return m;
  if (provider === 'zhipu') return process.env.ZHIPU_MODEL?.trim() || defaultModel(provider);
 ...
 */
export function resolveEffectiveLlmConfig(db: Db, userId: string): EffectiveLlmConfig {
  const row = db
    .prepare(
      `select user_id, parse_mode, llm_provider, llm_api_key, llm_base_url, llm_model, updated_at_ms
       from user_parse_settings where user_id = ?`
    )
    .get(userId) as UserParseSettingsRow | undefined;
...
/** 连接测试：合并数据库/环境变量生效配置与请求体中的临时覆盖（未保存的 Key 等）。 */
export type LlmPingOverrides = {
  apiKey?: string | null;
...
export function resolveEffectiveLlmConfigForPing(db: Db, userId: string, overrides?: LlmPingOverrides): EffectiveLlmConfig {
  const eff = resolveEffectiveLlmConfig(db, userId);
  const provider = overrides?.llmProvider ?? eff.provider;

  const apiKey =
    (typeof overrides?.apiKey === 'string' && overrides.apiKey.trim().length > 0 ? overrides.apiKey.trim() : null) ||
    eff.apiKey?.trim() ||
    null;
...
export function maskApiKeyHint(key: string | null | undefined): { hasApiKey: boolean; hint: string | null } {
  const k = key?.trim();
  if (!k) return { hasApiKey: false, hint: null };
  if (k.length <= 6) return { hasApiKey: true, hint: '****' };
  return { hasApiKey: true, hint: `${k.slice(0, 3)}…${k.slice(-4)}` };
}

export function getParseSettingsForApi(db: Db, userId: string) {
  const row = db
    .prepare(
      `select user_id, parse_mode, llm_provider, llm_api_key, llm_base_url, llm_model, updated_at_ms
       from user_parse_settings where user_id = ?`
