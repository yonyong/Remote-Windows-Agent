...
function usageFromGemini(data: any, durationMs: number): LlmUsageSnapshot | null {
  const um = data?.usageMetadata;
  if (!um || typeof um !== 'object') return { durationMs };
  return {
    durationMs,
...
export async function geminiGenerateText(args: {
  ...
  /** 指令解析路径建议 application/json，约束模型只出 JSON */
  responseMimeType?: 'text/plain' | 'application/json';
}): Promise<{ text: string; usage: LlmUsageSnapshot | null }> {
  const t0 = Date.now();
  ...
  try {
    ...
    args.diagnosticLog?.warn?.(
      { phase: 'gemini_response_json', responseBody: clipForLlmLog(rawText, logMax) },
  ...
  const texts = parts.map((p: any) => (typeof p?.text === 'string' ? p.text : '')).join('');
  if (!texts.trim()) {
    args.diagnosticLog?.warn?.(
      {
...
export async function geminiPing(args: { apiKey: string; model: string }): Promise<void> {
  const url = geminiGenerateUrl(args.model, args.apiKey);
  const res = await fetch(url, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
> {
  const logParseFail = (t: string, err: unknown) => {
    const logMax = llmLogBodyMaxChars();
    args.diagnosticLog?.warn?.(
      {
  ...
  let text = r1.text;
  let usage: LlmUsageSnapshot | null = r1.usage;
