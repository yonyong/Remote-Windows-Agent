...
import {
  appendExecutionResultToInterpretation,
  assertDeviceOwned,
  buildChatHistoryForLlm,
  insertDeviceChatMessage,
  listDeviceChatMessages,
  DEVICE_CHAT_PAGE_MAX
...
async function requireAuthUser(request: any, reply: any) {
  try {
  ...
  if (!row) {
    return reply.code(401).send({
      error: 'session_stale',
      hint:
        'This token user id no longer exists in the database (common after deleting control-plane.sqlite). Log out in the web UI and register/login again.'
    });
  }
}
...
app.post('/auth/register', async (req, reply) => {
  ...
  const existing = db.prepare('select id from users where lower(trim(email)) = ?').get(email) as any;
...
app.post('/auth/login', async (req, reply) => {
  ...
  const hint =
    '账号或密码不匹配。若电脑能登录而手机不能，多半是手机连到了另一台控制面（空库）：请在登录页的「控制面 API」填写与电脑相同的地址（例如 http://电脑IP:8787）。';

  let user: { id: string; email: string; password_hash: string } | undefined;

  if (raw.includes('@')) {
    const email = normalizeEmail(raw);
    user = db.prepare('select id, email, password_hash from users where lower(trim(email)) = ?').get(email) as any;
    ...
    const rows = db
      ...
      .all(local) as { id: string; email: string; password_hash: string }[];
    ...
    if (matches.length === 1) {
      ...
      return reply.code(409).send({
        ...
        hint:
          '该邮箱前缀对应多个账号，请改用完整邮箱（含 @ 域名）登录，例如 admin@qq.com。'
      });
    } else {
      return reply.code(401).send({ error: 'invalid_credentials', hint });
    }
  ...
  const token = app.jwt.sign({ sub: user!.id, email: user!.email } satisfies JwtPayload, { expiresIn: '12h' });
...
function persistCommandLlmUsage(commandId: string, usage: LlmUsageSnapshot | null | undefined) {
...
const agentsByDeviceId = new Map<string, AgentConn>();
const agentsByPairingCode = new Map<string, AgentConn>();

function sendJson(ws: any, obj: unknown) {
...
app.get('/ws/agent', { websocket: true }, (conn, req) => {
  ...
  if (pairingCode) {
    ...
    const device = db.prepare('select id, revoked_at_ms from devices where device_token = ?').get(deviceToken) as any;
  ...
  ws.on('message', (raw: any) => {
    try {
      const msg = JSON.parse(String(raw)) as AgentToServer;
      const commandExists = (commandId: string) =>
        Boolean(db.prepare('select 1 as ok from commands where id = ?').get(commandId) as any);
      ...
      if (msg.type === 'event') {
        ...
        const shotCmd = db
          ...
          .get(msg.commandId) as { user_id: string; device_id: string } | undefined;
        if (shotCmd && msg.pngBase64) {
          const dup = db
            .prepare(
              `select 1 as ok from device_chat_messages where command_id = ? and kind = 'screenshot' and attachment_base64 = ? limit 1`
            )
            .get(msg.commandId, msg.pngBase64) as { ok: 1 } | undefined;
          if (dup) {
  ...
  ws.on('close', () => {
    if (pairingCode) agentsByPairingCode.delete(pairingCode);
    if (agentConn.deviceId) agentsByDeviceId.delete(agentConn.deviceId);
  });
});
...
app.put('/me/parse-settings', { preHandler: requireAuthUser }, async (req, reply) => {
  ...
  const prev = db.prepare('select llm_api_key from user_parse_settings where user_id = ?').get(userId) as
    | { llm_api_key: string | null }
    | undefined;

  let nextKey: string | null = prev?.llm_api_key ?? null;
  if (body.clearApiKey) nextKey = null;
  ...
  const now = Date.now();
  db.prepare(
    `insert into user_parse_settings (user_id, parse_mode, llm_provider, llm_api_key, llm_base_url, llm_model, updated_at_ms)
     values (?, ?, ?, ?, ?, ?, ?)
  ...
  ).run(userId, body.parseMode, body.llmProvider, nextKey, baseStored, body.llmModel.trim(), now);
...
app.post('/me/parse-settings/test', { preHandler: requireAuthUser }, async (req, reply) => {
  ...
  if (!eff.apiKey?.trim()) {
    return reply.code(400).send({
      error: 'no_api_key',
      hint:
        '没有可用的 API Key：请先在「API Key」输入框填写并点击「保存解析设置」，或直接在测试请求里带上未保存的 Key。也可在服务器环境变量中配置对应厂商的密钥。'
    });
  }
  try {
...
app.get('/devices/:deviceId/chat', { preHandler: requireAuthUser }, async (req, reply) => {
  ...
  if (typeof limStr === 'string' && limStr.trim()) {
    ...
    if (Number.isFinite(n)) limit = Math.min(DEVICE_CHAT_PAGE_MAX, Math.max(1, n));
  }
  if (!assertDeviceOwned(db, deviceId, userId)) return reply.code(404).send({ error: 'device_not_found' });
...
app.post('/devices/:deviceId/revoke', { preHandler: requireAuthUser }, async (req, reply) => {
  const userId = getUserId(req);
  const deviceId = (req.params as any).deviceId as string;
  const device = db.prepare('select id from devices where id = ? and owner_user_id = ?').get(deviceId, userId) as any;
...
app.post('/devices/:deviceId/commands', { preHandler: requireAuthUser }, async (req, reply) => {
  ...
  const device = db.prepare('select id from devices where id = ? and owner_user_id = ? and revoked_at_ms is null').get(deviceId, userId) as any;
  if (!device) return reply.code(404).send({ error: 'device_not_found' });
...
app.post('/commands/:commandId/approve', { preHandler: requireAuthUser }, async (req, reply) => {
  const userId = getUserId(req);
  const commandId = (req.params as any).commandId as string;
  const cmd = db.prepare('select * from commands where id = ? and user_id = ?').get(commandId, userId) as any;
  if (!cmd) return reply.code(404).send({ error: 'command_not_found' });
  if (cmd.status !== 'needs_approval') return reply.code(409).send({ error: 'not_approvable', status: cmd.status });
