export type AgentWallpaper = 'slate' | 'emerald' | 'violet' | 'amber';
...
export function getBuiltinAgentById(id: string): BuiltinAgentDefinition | null {
  return BUILTIN_AGENTS.find((a) => a.id === id) ?? null;
}

export function buildAgentSystemPrompt(agent: BuiltinAgentDefinition, personaAddon: string | undefined): string {
  const add = personaAddon?.trim();
  if (!add) return agent.systemPrompt;
...
export function buildCustomAgentSystemPrompt(displayName: string, personaAddon: string | undefined): string {
